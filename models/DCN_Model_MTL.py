...
class DCN_Model_MTL(Model):
    def __init__(self, feat_columns, emb_size=5, cin_layers=[7,15]):
        ...
        if self.seq_feats:
            self.seq_embeds = [layers.Embedding(input_dim=feat['feat_num'], output_dim=emb_size, mask_zero=True) for
                               feat in self.seq_feats]

        # DCN Cross Network
        # 这里 input_dim = dense + sparse_embedding 的拼接维度。
        if self.seq_feats:
            self.cross_network  = CrossNetwork(input_dim=len(self.dense_feats) + len(self.sparse_feats) * emb_size + len(self.seq_feats) * emb_size, num_layers=3)
        else:
            self.cross_network  = CrossNetwork(input_dim=len(self.dense_feats) + len(self.sparse_feats) * emb_size,num_layers=3)
    ...
    def call(self, inputs, training=False):
        ...
        if self.seq_feats:
            ...
            common_input = tf.concat([dense_inputs, sparse_flatten_embeddings, seq_flatten_embeddings],axis=1)  # shape=(2, X) + shape=(2, Y) + shape=(2, Z)  => shape=(2, 17))
        else:
            # sparse、dense进行拼接
            common_input = tf.concat([dense_inputs, sparse_flatten_embeddings], axis=1)

        # 第二部分： DNN
        dnn_output = self.dnn(common_input, training=training)

        # 第三部分： DCN-CrossNetwork
        dcn_output = self.dense_cross_layer(self.cross_network(common_input))       # 入参是self.cross_network(dcn_input) => (3, 17)
...
if __name__ == '__main__':
    ...
    if not use_sequence:
        ...
        model = DCN_Model_MTL(feat_columns=feat_columns, emb_size=5)
