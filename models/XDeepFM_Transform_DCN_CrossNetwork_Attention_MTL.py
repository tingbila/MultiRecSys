...
# 定义XDeepFM + Transform当中的Attention + CrossNetwork_Attention（CrossNetwork中进行Attention创新）
class XDeepFM_Transform_DCN_CrossNetwork_Attention_MTL(Model):
    def __init__(self, feat_columns, emb_size=5, cin_layers=[7,15]):
        ...
        # DCN Cross Network-这里进行了创新
        self.cross_network = VectorWiseAttentionCross(num_layers=3,embed_dim=self.emb_size)
        ...
        self.pooled = tf.keras.layers.GlobalAveragePooling1D()
        """
        GlobalAveragePooling1D 就是在 time_steps 维度(第2个维度)上求平均值:
        x = tf.constant([
            [[1., 2., 3.],     # time step 1
             [4., 5., 6.],     # time step 2
             [7., 8., 9.]],    # time step 3
        ...
        对第一个样本 [[1,2,3],[4,5,6],[7,8,9]]：对每一列（特征维度）求平均：
        第1维：(1 + 4 + 7) / 3 = 4.0
        第2维：(2 + 5 + 8) / 3 = 5.0
        第3维：(3 + 6 + 9) / 3 = 6.0
        所以第一个样本输出为 [4.0, 5.0, 6.0] 第二个样本同理。        
        """
    ...
    def call(self, inputs, training=False):
        ...
        # 第二部分：FM部分
        sparse_embeds = tf.stack([emb(sparse_inputs[:, i]) for i, emb in enumerate(self.fm_sparse_embeds)],
                                 axis=1)  # shape: (batch_size, num_sparse_fields, emb_dim)
        ...
        if self.seq_feats:
            ...
            for i, (seq_input, seq_layer) in enumerate(zip(seq_inputs, self.seq_embeds)):
                ...
                pooled = tf.reduce_mean(seq_emb, axis=1,
                                        keepdims=True)  # (batch_size, 1, emb_dim)  从这可以看出边长序列的字段数据最终整体也是当成一个字段处理
            ...
            fm_input = tf.concat([sparse_embeds, seq_embeds_concat],
                                 axis=1)  # (batch, num_sparse_fields + num_seq_fields, emb_dim)
        else:
        ...
        # DCN部分的输入：[2,3,4]这种格式输入
        dcn_input  = fm_input
        cross_network_output = self.cross_network(dcn_input)      # [2,3,4]
        pooled_output        = self.pooled(cross_network_output)  # [2,4]
        dcn_output           = self.dense_cross_layer(pooled_output)
        ...
        # 合成最终输出
        logits = first_order_output + second_order +  cin_output + dnn_output + pooled_attention_out + dcn_output
...
if __name__ == '__main__':
    ...
    if not use_sequence:
        ...
        model = XDeepFM_Transform_DCN_CrossNetwork_Attention_MTL(feat_columns=feat_columns, emb_size=5)
