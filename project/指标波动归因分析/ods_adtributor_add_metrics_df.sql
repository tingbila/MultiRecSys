            ...
            from (
                  ...
                  from (
                        select
                              ...
                              sum(ep) over (partition by dim order by surprise_rank asc rows between unbounded preceding and current row ) as ep_sum
                        from (
                              select
                                    t4.dim,
                                    t4.element,
                                    ...
                                    ROUND((after - before) / (aft_sum - pre_sum), 6) as ep
                              from (
                                    select
                                          t3.dim,
                                          t3.element,
                                          ...
                                          ROUND(0.5 * (p * LN(2 * p / (p + q)) / LN(10) + q * LN(2 * q / (p + q)) / LN(10)), 6)  as surprise
                                    from (
                                          select
                                                t1.dim,
                                                t1.element,
                                                ...
                                                ROUND(t1.before / t2.pre_sum, 6) AS p,
                                                ROUND(t1.after / t2.aft_sum, 6)  AS q
                                          from  base_info t1
                                          left  join (
                                                ...
                                                select
                                                      ...
                                                      'on' as join_column
                                                from  base_info
                                                where dim = '渠道'   -- 选择一个渠道即可，因为所有维度的sum(after)和sum(before)是一样的
                                          ) t2
