...
from (
      SELECT
            ...
            MAX(IF(dim = '分子', surprise_rank, null))               AS m1_surprise_rank,
            MAX(IF(dim = '分子', ep, null))                          AS m1_ep,
            MAX(IF(dim = '分子', ep_sum, null))                      AS m1_ep_sum,
            MAX(IF(dim = '分子', lag_ep_sum, null))                  AS m1_lag_ep_sum,
            MAX(IF(dim = '分子', surprise_sum, null))                AS m1_surprise_sum,
            -- 分母
            ...
            MAX(IF(dim = '分母', surprise_rank, null))               AS m2_surprise_rank,
            MAX(IF(dim = '分母', ep, null))                          AS m2_ep,
            MAX(IF(dim = '分母', ep_sum, null))                      AS m2_ep_sum,
            MAX(IF(dim = '分母', lag_ep_sum, null))                  AS m2_lag_ep_sum,
            MAX(IF(dim = '分母', surprise_sum, null))                AS m2_surprise_sum
      from (
            select
                  dim,
                  element,
                                    ...
                                    from (
                                          ...
                                          left  join (
                                                ...
                                                select
                                                      ...
                                                      'on' as join_column
                                                from  base_info
                                                -- where dim = '分发模块'   -- 选择一个渠道即可，因为所有维度的sum(after)和sum(before)是一样的
                                                group by dim
                                          ) t2
                                          on t1.dim = t2.dim
                                    ) t3
