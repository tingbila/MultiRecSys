...
def create_dataset(file_path='./data/criteo_sampled_data.csv', embed_dim=5):
    ...
    """
    column_names = ["platform", "app_name", "app_version", "country", "region", "language","channel", "create_date", "active_last_date", "target"]
    data = pd.read_csv(file_path, sep=',', names=column_names)  # 大数据集
    print(data.head(5))

    # 区分数值特征和稀疏特征
    dense_feats    = []
    sparse_feats   =  ["platform", "app_name", "app_version", "country", "region", "language","channel", "create_date", "active_last_date"]
    ...
    def df_to_dataset(df):
        ...
        labels = tf.convert_to_tensor(df[["target"]].values, dtype=tf.float32)
        ...
        return tf.data.Dataset.from_tensor_slices((input_features, {'finish': labels_finish}))
