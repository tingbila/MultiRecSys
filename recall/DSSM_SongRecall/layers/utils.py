...
def combined_dnn_input_new(sparse_embedding_list, dense_value_list):
    if len(sparse_embedding_list) > 0 and len(dense_value_list) > 0:
        sparse_dnn_input = Flatten()(tf.concat(sparse_embedding_list, -1))
        dense_dnn_input = Flatten()(tf.concat(dense_value_list, -1))
        print(sparse_embedding_list, dense_value_list)
        return tf.concat([sparse_dnn_input, dense_dnn_input], -1)
    elif len(sparse_embedding_list) > 0:
        return Flatten()(tf.concat(sparse_embedding_list, -1))
    elif len(dense_value_list) > 0:
        return Flatten()(tf.concat(dense_value_list, -1))
    else:
        raise NotImplementedError("dnn_feature_columns can not be empty list")
