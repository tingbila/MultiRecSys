...
from deepctr.feature_column import SparseFeat, VarLenSparseFeat, DenseFeat, varlen_embedding_lookup, \
    get_varlen_pooling_list, get_dense_input, build_input_features, embedding_lookup
...
from layers.core import CapsuleLayer, PoolingLayer, MaskUserEmbedding, LabelAwareAttention, SampledSoftmaxLayer, \
    EmbeddingIndex
from layers.utils import NoMask, combined_dnn_input, concat_func
...
def MIND(user_feature_columns, item_feature_columns, k_max=2, p=100, dynamic_k=False,
    ...
    for fc in varlen_sparse_feature_columns:  # 浏览历史数据处理
    ...
    history_emb = PoolingLayer()(keys_emb_list)
    target_emb = PoolingLayer()(query_emb_list)
    # print(history_emb)
    ...
    # print(user_deep_input)
    user_embeddings = DNN(user_dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout,
                          use_bn=dnn_use_bn, output_activation=output_activation,seed=seed)(user_deep_input)
    ...
    user_embedding_final = l2_normalize(user_embedding_final)
