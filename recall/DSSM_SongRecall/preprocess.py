...
def gen_data_set(data, negsample=0):
    ...
    train_set = []
    test_set = []
    # print(data.groupby('user_id').head())
    for reviewerID, hist in tqdm(data.groupby('user_id')):
        pos_list = hist['movie_id'].tolist()    # 用户浏览过的item_id
        rating_list = hist['rating'].tolist()    # 用户对item的评分
        ...
        for i in range(1, len(pos_list)):
            ...
            if i != len(pos_list) - 1:
                ...
                for negi in range(negsample):
                    ...
                    if i < 2:
                        print(reviewerID, negi, i*negsample+negi, neg_list[i*negsample+negi])
            else:
...
def gen_model_input(train_set, user_profile, seq_max_len):
    ...
    # padding
    train_seq_pad = tf.keras.preprocessing.sequence.pad_sequences(train_seq, maxlen=seq_max_len, padding='post', truncating='post', value=0)
    # print(train_seq_pad)
...
def gen_model_input_sdm(train_set, user_profile, seq_short_len, seq_prefer_len):
    ...
    train_short_item_pad = tf.keras.preprocessing.sequence.pad_sequences(short_train_seq, maxlen=seq_short_len, padding='post', truncating='post',
                                        value=0)
    train_prefer_item_pad = tf.keras.preprocessing.sequence.pad_sequences(prefer_train_seq, maxlen=seq_prefer_len, padding='post', truncating='post',
                                         value=0)
    train_short_genres_pad = tf.keras.preprocessing.sequence.pad_sequences(short_train_seq_genres, maxlen=seq_short_len, padding='post', truncating='post',
                                        value=0)
    train_prefer_genres_pad = tf.keras.preprocessing.sequence.pad_sequences(prefer_train_seq_genres, maxlen=seq_prefer_len, padding='post', truncating='post',
                                        value=0)
