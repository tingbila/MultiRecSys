...
/home/worker/anaconda3/bin/python3.7 /data1/guifang.ji/DSSM_SongRecall/u2i_sdm.py
if [ $? != 0 ]; then
    echo "u2i_dssm faild"
#    curl "http://devops-callback.ushow.media:9910/v1/api/message?title=u2i_youtube_faild&toUser=ZaiLuShang&text=$day"
    curl -H "Content-Type:application/json" -X POST -d '{"type": "Card", "touser": ["guifang.ji"], "id": 5, "data": {"title": "u2i_sdm_faild", "msg": "u2i_sdm_faild"}}' https://devops.ushow.media/devops-goserver-v1/notify/message/
    exit 1
fi

echo "write pika start"
/data/anaconda2/bin/python /data1/guifang.ji/DSSM_SongRecall/write_pika_pipeline.py "song:sdmU2i:%s" "/data1/guifang.ji/DSSM_SongRecall/data/sdm_data_u2i.txt"
if [ $? != 0 ]; then
